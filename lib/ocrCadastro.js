// Gera os JPG de OCR a partir dos CR2 e copia pra pasta do Cadastro.
//
...
// Lista os RAW do GTIN, na raiz e nas subpastas de tag. Devolve [{ caminho, nome, subpasta }].
function listarRaws(os, gtin) {
    ...
    const varrer = (dir, subpasta) => {
        ...
        for (const nome of fs.readdirSync(dir)) {
            ...
            if (fs.statSync(caminho).isFile() && ehRaw(nome)) {
                achados.push({ caminho, nome, subpasta });
            }
        }
    };
...
function gerarOcr(os, gtin, arquivos, config) {
    const cfg = config || carregarConfig();
    const raws = listarRaws(os, gtin);

    if (raws.length === 0) {
        return { ok: false, erro: 'Nenhum RAW encontrado em Finalizadas\\OS_' + os + '\\' + gtin, gerados: [], falhas: [] };
    }
    ...
    if (Array.isArray(arquivos) && arquivos.length > 0) {
        ...
        alvos = raws.filter(r => bases.has(nomeBase(r.nome).toLowerCase()));
        if (alvos.length === 0) {
            return { ok: false, erro: 'Nenhum RAW casou com os arquivos pedidos', gerados: [], falhas: [] };
        }
    } else {
        ...
        const marcadas = raws.filter(r => temMarcaOcr(r.nome));
    ...
    for (const raw of alvos) {
        const base = nomeBase(raw.nome);
        // Destino PRINCIPAL: a estrutura OCR local. Espelha a subpasta (RT/IS/AP) do lado RAW.
        const destinoDir = pastaOcrDestino(os, gtin, raw.subpasta);
        // O _ocr e marcador nosso, interno - o arquivo de OCR sai com o nome limpo.
        const baseSaida = temMarcaOcr(raw.nome) ? base.slice(0, -SUFIXO_OCR.length) : base;
        ...
        try {
            ...
            const preview = extrairPreview(raw.caminho);
            if (preview.ok) {
                fs.writeFileSync(destinoArq, preview.buffer);
                gerados.push({ arquivo: nomeSaida, origem: 'preview-embutido', larg: preview.larg, alt: preview.alt, kb: Math.round(preview.tam / 1024) });
            } else {
                falhas.push({ arquivo: raw.nome, motivo: preview.motivo });
                continue;
            }

            const copia = copiarParaCadastro(cfg, os, gtin, raw.subpasta, destinoArq, nomeSaida);
            ...
            falhas.push({ arquivo: raw.nome, motivo: err.message });
        }
    }

...
function inferirCampos(pastaGtinPath) {
    ...
    // _coding marca foto de referencia pro editor, nao e produto - nunca entra na
    // contagem que vai pro Redmine (QTD_IMAGENS_MOCKUP/RECORTE), com ou sem subpasta.
    ...
    // Mockup e o unico sinal certo de Brasil (Virafilme) - nunca vai pra EUA. Mas se
    // TODAS as fotos da raiz forem _coding, nao sobrou nenhum produto de verdade pra
    // contar - mesmo com a subpasta marcada, fica indefinido em vez de mandar "0".
    if (subpastas.temMockup) {
        if (semCoding.length === 0) {
            return {
                destino: 'indefinido',
                motivo: 'Pasta com subpasta Mockup mas todas as fotos da raiz estao marcadas _coding - nenhuma foto de produto pra contar',
                campos: {}
            };
        }
        const campos = {};
        campos[CF.QTD_IMAGENS_MOCKUP] = String(semCoding.length);
    ...
    if (subpastas.temRecorte) {
        if (semCoding.length === 0) {
            return {
                destino: 'indefinido',
                motivo: 'Pasta com subpasta Recorte mas todas as fotos da raiz estao marcadas _coding - nenhuma foto de produto pra contar',
                campos: {}
            };
        }
        const campos = {};
